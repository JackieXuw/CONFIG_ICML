...
def get_epbo_result(problem_config, plot=False):
...
def run_one_instance(x):
    ...
    if True:
        ...
        epbo_costs, epbo_objs, epbo_opt, epbo_obj_traj, epbo_constrs\
            = get_epbo_result(problem_config)

        safe_costs, safe_objs, safe_opt, safe_obj_traj, safe_constrs\
            = 0, 0, 0, 0, 0#gget_safe_bo_result(problem_config, plot=False)
        con_costs, con_objs, con_opt, con_obj_traj, con_constrs\
            = 0, 0, 0, 0, 0#gget_constrained_bo_result(problem_config, plot=False)
        pdbo_costs, pdbo_objs, pdbo_opt, pdbo_obj_traj, pdbo_constrs\
            = 0, 0, 0, 0, 0#gget_pdbo_result(problem_config)
    ...
    return safe_costs, safe_objs, con_costs, con_objs, vabo_costs_tmp, \
        ...
        lcb2_obj_traj, lcb2_constrs, problem_config, epbo_costs, epbo_objs, \
        epbo_opt, epbo_obj_traj, epbo_constrs
...
for run_id in [8] * num_runs:
    safe_costs, safe_objs, con_costs, con_objs, vabo_costs_tmp, \
        ...
        vabo_obj_traj_tmp, vabo_constrs_tmp, lcb2_costs, lcb2_objs, lcb2_opt, \
        lcb2_obj_traj, lcb2_constrs, problem_config, epbo_costs, epbo_objs, \
        epbo_opt, epbo_obj_traj, epbo_constrs = run_one_instance(run_id)
    multi_results.append((safe_costs, safe_objs, con_costs, con_objs,
                         ...
                         safe_obj_traj, safe_constrs, con_obj_traj,
                          ...
                          problem_config, epbo_costs, epbo_objs, epbo_opt, \
                          epbo_obj_traj, epbo_constrs))
    ...
    for safe_costs, safe_objs, con_costs, con_objs, vabo_costs_tmp, vabo_objs_tmp,\
        ...
        lcb2_constrs, problem_config, epbo_costs, epbo_objs, epbo_opt, \
        epbo_obj_traj, epbo_constrs in multi_results:
        if safe_costs is not None:
            epbo_cost_lists.append(epbo_costs)
            epbo_simple_regret_lists.append(
                np.array(epbo_objs)-problem_config['f_min'])
            epbo_regret_lists.append(
                np.array(epbo_obj_traj)-problem_config['f_min']
            )
            epbo_constrs_lists.append(
                np.array(epbo_constrs)
            )


    epbo_cost_arr = np.mean(np.array(epbo_cost_lists), axis=0)
    epbo_simple_regret_arr = np.mean(np.array(epbo_simple_regret_lists), axis=0)
    epbo_regret_arr = np.mean(np.array(epbo_regret_lists), axis=0)
...
np.savez(f'./result/test_Prob_set_result_with_pdbo_{now_time_str}',
         epbo_regret_lists, epbo_constrs_lists)
