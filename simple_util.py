...
def get_config(problem_name, problem_dim=None, gp_kernel=None, init_points_id=0):
    ...
    if problem_name == 'GP_sample_two_funcs_epsilon_infeasible':
        if problem_dim is None:
            problem_dim = 1
        if gp_kernel is None:
            gp_kernel = 'Gaussian'
        ...
        min_constr_val = np.min(constr_func(parameter_set))
        inf_epsilon = 0.1
    ...
    if problem_name == 'energym_apartment_therm_tune_PH':
        if problem_dim is None:
            problem_dim = 3
        if gp_kernel is None:
            # gp_kernel = 'Gaussian'
        ...
        config['discretize_num_list'] = [100] * problem_dim
        config['num_constrs'] = 1
        config['bounds'] = [(0.01, 0.3), (15, 20), (4, 8)]
        #[(0.05, 0.95), (18, 25)] #[(0.05, 0.95), (0.05, 0.95)] # [(0.05, 0.45), (0.5, 0.95)] #[(-10, 10)] * problem_dim
        config['train_X'] = safeopt.linearly_spaced_combinations(
            config['bounds'],
        ...
        if gp_kernel == 'Gaussian':
            kernel = GPy.kern.RBF(input_dim=len(config['bounds']), variance=1.,
                                  lengthscale=[0.3/3.0, 2.0/3.0, 4.0/3.0],
                                  ARD=True)
            constr_kernel = GPy.kern.RBF(
                input_dim=len(config['bounds']), variance=1.,
                lengthscale=[0.3/3.0, 2.0/3.0, 4.0/3.0], ARD=True)
        ...
        if gp_kernel == 'Matern52':
            kernel = GPy.kern.Matern52(
                input_dim=len(config['bounds']), variance=4.,
                lengthscale=[2.7755e-07, 6.8812e-01, 1.3061e+01],
                ARD=True)
            constr_kernel = GPy.kern.Matern52(
                input_dim=len(config['bounds']), variance=0.56,
                lengthscale=[ 0.0998,  6.6366, 10.2420],
                ARD=True)

            constr_kernel_1 = GPy.kern.Matern52(
                input_dim=len(config['bounds']), variance=1.,
                lengthscale=[ 0.0998,  6.6366, 10.2420], ARD=True)
        ...
        # Initial safe point
        x0 = np.array([[0.3, 20, 4.0]])

        def f(x, simulator_to_use=None):
            energy_mean = 808 #1117.9042996350206 #1500.954861111111  # 385.3506855413606
            energy_std = 311 #112.68479944833072 # 12.634541238981747 # 15.832128159177238
            dev_mean =  3.75 #1.2810264538593588 * 3.0 #0.31793574850163836 # 0.48119964518630765
            dev_std =  2.56 #0.7189539259567139 #0.03637075568519829 # 0.016940298884339722
            ...
            for k in range(size_batch):
                energy, dev = get_ApartTherm_kpis(
                    controller='PH', params=(x[k, 0], x[k, 1], x[k, 2]))
        ...
        def g_1(x, simulator_to_use=None):
            energy_mean = 808 #1117.9042996350206 #1500.954861111111  # 385.3506855413606
            energy_std = 311 #112.68479944833072 # 12.634541238981747 # 15.832128159177238
            dev_mean =  3.75 #1.2810264538593588 #0.31793574850163836 # 0.48119964518630765
            dev_std = 2.56 #0.7189539259567139 #0.03637075568519829 # 0.016940298884339722
            size_batch, _ = x.shape
            energy_list = []
            dev_list = []
