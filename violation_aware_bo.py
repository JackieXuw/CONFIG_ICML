...
class ViolationAwareBO:
    ...
    def setup_optimizer(self):
        ...
        if self.normalize_input:
            self.gp_obj_mean = np.mean(self.init_obj_val_arr)
        else:
            self.gp_obj_mean = 0

        self.gp_obj = GPy.models.GPRegression(self.x0_arr,
                                              self.init_obj_val_arr-self.gp_obj_mean,
                                              self.kernel_list[0],
        ...
        for i in range(self.opt_problem.num_constrs):
            ...
            self.gp_constr_list.append(
                GPy.models.GPRegression(self.x0_arr,
                                        np.expand_dims(
                                            self.init_constr_val_arr[:, i], axis=1)-gp_constr_mean,
        ...
        opt_problem = self.opt_problem
        #self.gps_torch = gp_model.BOGPModels(opt_problem.train_X, opt_problem.train_obj-np.mean(opt_problem.train_obj),
        #                                     opt_problem.train_constr-np.mean(opt_problem.train_constr, axis=0),
        #                                     self.x0_arr, self.init_obj_val_arr-self.gp_obj_mean,
        #                                     self.init_constr_val_arr - np.mean(self.init_constr_val_arr, axis=0)
        #                                     )
    ...
    def get_acquisition(self, type='budget_aware_EIc', gp_package='gpy', lcb_beta=3):
        if gp_package == 'gpytorch':
            gps_torch = self.gps_torch
            gps_torch.infer_gp_obj_torch.eval()
            gps_torch.infer_obj_likelihood.eval()
            obj_predict = gps_torch.infer_obj_likelihood(gps_torch.infer_gp_obj_torch(
                torch.from_numpy(self.parameter_set)))
            obj_mean = obj_predict.mean.detach().numpy()
            obj_var = obj_predict.variance.detach().numpy()
        else:
        ...
        for i in range(self.opt_problem.num_constrs):
            if gp_package == 'gpytorch':
                gps_torch = self.gps_torch
                gps_torch.infer_gp_constr_torch_list[i].eval()
                gps_torch.infer_constr_likelihood_list[i].eval()
                constr_predict = gps_torch.infer_constr_likelihood_list[i](gps_torch.infer_gp_constr_torch_list[i](
                    torch.from_numpy(self.parameter_set)))
                mean = constr_predict.mean.detach().numpy()
                var = constr_predict.variance.detach().numpy()
            else:
        ...
        self.S = self.parameter_set[(prob_all_not_use_up_budget >= 1 - self.prob_eps)]
    ...
    def get_beta(self):
        return min(max(self.curr_eval_budget, 1), self.beta_func(self.curr_eval_budget))
    ...
    def make_step(self, update_gp=False, gp_package='gpy'):
        ...
        if self.normalize_input:
            ...
            #self.gps_torch.set_new_datas(new_X, new_obj, new_constrs_list, update_hyper_params=True)
        else:
