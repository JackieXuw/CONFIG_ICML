...
class OptimizationProblem:
    ...
    def parameter_estimation(self, kernel='Gaussian'):
        # dimension-wise parameter estimation
        var_dim = self.var_dim
        medium_input = [(self.bounds[i][0] + self.bounds[i][1]) * 0.5 for i in range(self.var_dim)]
        ...
        for dim_i in range(var_dim):
            ...
            for k in range(self.discretize_num_list[dim_i]+1):
                # set X_i to be the k-th value while setting others to be middle value
                X = medium_input
                X[dim_i] = self.bounds[dim_i][0] + (k/self.discretize_num_list[dim_i]) * \
                           (self.bounds[dim_i][1] - self.bounds[dim_i][0])
                objY, constrYs = self.sample_point(np.array([X]), reset_init=True)
                train_X_list.append(copy.deepcopy(X))
                train_objY_list.append(objY)
                for kk in range(self.num_constrs):
                    train_constrY_list[kk].append(constrYs[kk, :])
            ...
            plt.title(str(dim_i))
            parameters_list[0].append(self.get_1d_kernel_params(train_X[:, dim_i], train_objY[:, 0, 0], kernel))
            for i in range(self.num_constrs):
                parameters_list[i+1].append(self.get_1d_kernel_params(train_X[:, dim_i], train_constrY[i, :, 0], kernel))
